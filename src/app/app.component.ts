...
export class AppComponent implements OnInit, OnDestroy {
  ...
  initCharacterEvents(): void {
    ...
    this.charactersResults$ = this.searchTermByCharacters.pipe(
      // YOUR CODE STARTS HERE
      filter(srcQuery => srcQuery.length >= 3),
      debounceTime(500),
  ...
  loadCharactersAndPlanet(): void {
    ...
    this.planetAndCharactersResults$ = forkJoin([this.mockDataService.getCharacters(), this.mockDataService.getPlanets()]).pipe(
      map(([characterArr, planetArr]) => 
        [...characterArr, ...planetArr]
      )
    );
    // YOUR CODE ENDS HERE
  }
  ...
  initLoadingState(): void {
    ...
    // YOUR CODE STARTS HERE
    const combinedData = combineLatest([this.mockDataService.getCharactersLoader(), this.mockDataService.getPlanetLoader()]).subscribe(
